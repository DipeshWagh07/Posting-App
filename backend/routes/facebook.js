...
router.get("/callback", async (req, res) => {
  ...
  if (fbError) {
    ...
    return res.redirect(
      `http://localhost:3000/auth-error?provider=facebook&error=${encodeURIComponent(
        errorReason || "unknown"
      )}&state=${state || ""}`
    );
  }
  ...
  try {
    ...
    res.redirect(
      ...
      )}&code=${errorDetails.type}&state=${state || ""}`
    );
  }
});
  ...
  try {
    ...
    if (!page.access_token) {
      return res.status(403).json({
        ...
        solution:
          "Re-authenticate with Facebook and ensure all permissions are granted",
      });
    }
    ...
    res.status(statusCode).json({
      ...
      code: errorDetails.type,
      solution:
        errorDetails.solution || "Check permissions and content guidelines",
    });
  }
});
