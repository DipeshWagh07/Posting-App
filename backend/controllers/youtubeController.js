...
export const youtubeCallback = async (req, res) => {
  ...
  if (error) {
    ...
    const redirectUrl = `http://localhost:3000/auth/youtube/callback?error=${encodeURIComponent(
      error
    )}`;
  ...
  try {
    ...
    const redirectUrl = `http://localhost:3000/auth/youtube/callback?accessToken=${
      tokens.access_token
    }&refreshToken=${tokens.refresh_token}&channelId=${
      channelInfo.id
    }&channelName=${encodeURIComponent(channelInfo.snippet.title)}`;
